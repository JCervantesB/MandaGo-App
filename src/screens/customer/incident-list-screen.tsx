...
export function IncidentListScreen({ navigation }: IncidentListScreenProps) {
  ...
  const fetchIncidents = async () => {
    setIsLoading(true);
    try {
      const response = await fetch(`${API_BASE_URL}/incidents/client/list`, { credentials: 'include' });
      if (response.ok) {
    ...
    <View className="flex-1 bg-background">
      <SafeAreaView className="flex-1" edges={['top', 'left', 'right']}>
        ...
        >
          ...
          <Text className="text-3xl font-extrabold text-text mt-1 mb-4">Incidencias abiertas</Text>
              {openIncidents.map((incident) => (
                <Pressable
                  key={incident.id}
                  onPress={() => navigation.navigate('ClientIncidentDetail', { incidentId: incident.id })}
                  className="bg-white rounded-xl p-4 mb-3 border-l-4 border-red-500"
          ...
          {incidents.length === 0 && (
            <View className="flex-1 justify-center items-center pt-20">
              <AlertTriangle size={48} color={appColors.textMuted} />
              <Text className="text-base text-text/60 mt-4">No hay incidencias</Text>
            </View>
          )}
        </ScrollView>
      </SafeAreaView>
