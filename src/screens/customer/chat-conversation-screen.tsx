...
interface ClientChatConversationScreenProps {
  route: { params: { channelId: number; orderPublicId: string } };
  navigation: any;
}

export function ClientChatConversationScreen({ route, navigation }: ClientChatConversationScreenProps) {
      ...
      <View className={`flex-row ${isOwn ? 'justify-end' : 'justify-start'} mb-1 px-4`}>
        {!isOwn && showHeader && (
          <View className="w-8 h-8 rounded-full bg-primary/10 items-center justify-center mr-2 mt-1">
            <Text className="text-xs text-primary font-semibold">R</Text>
          </View>
        )}
        ...
        <View className="flex-row items-center px-4 py-4">
          ...
          <View className="flex-1 ml-3">
            ...
            <View className="flex-row items-center mt-0.5">
              ...
              <Text className="text-xs text-white/80 ml-1.5">Chat con tu repartidor</Text>
            </View>
          </View>
      ...
      {isLoading ? (
        ...
        <View className="flex-1 items-center justify-center px-8">
          ...
          <Text className="text-sm text-text-muted text-center mt-2">
            Cuando tu repartidor te contacte, verás aquí los mensajes de la conversación.
          </Text>
        </View>
      ) : (
        <FlatList
