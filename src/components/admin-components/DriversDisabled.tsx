...
interface DriversDisabledProps {
  onDataLoaded?: (count: number) => void;
}

// Columnas de la tabla de repartidores deshabilitados
...
// Lista de repartidores deshabilitados con tabla de datos
export function DriversDisabled({ onDataLoaded }: DriversDisabledProps) {
  ...
  const {
    ...
    goToNextPage,
    goToPreviousPage,
  } = useDriversList({ status: 'disabled' });
  ...
  return (
    <View className="flex-1 px-3 pt-3">
      <Text className="text-lg font-bold text-text mb-1">Repartidores deshabilitados</Text>
      <Text className="text-sm text-text/65 mb-3">
        {pagination.totalItems > 0
          ? `${pagination.totalItems} repartidores deshabilitados`
          : 'Cargando...'}
      </Text>
      ...
      <DataTable
        ...
        isLoading={isLoading}
        emptyMessage="No hay repartidores deshabilitados"
      />
    </View>
  );
}