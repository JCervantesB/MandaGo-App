...
interface DriversActiveProps {
  onDataLoaded?: (count: number) => void;
}

// Columnas de la tabla de repartidores activos
...
// Lista de repartidores activos con tabla de datos
export function DriversActive({ onDataLoaded }: DriversActiveProps) {
  ...
  return (
    <View className="flex-1 px-3 pt-3">
      <Text className="text-lg font-bold text-text mb-1">Repartidores activos</Text>
      <Text className="text-sm text-text/65 mb-3">
        {pagination.totalItems > 0
          ? `${pagination.totalItems} repartidores activos`
          : 'Cargando...'}
      </Text>
