...
interface ClientsActiveProps {
  onDataLoaded?: (count: number) => void;
}
// Columnas de la tabla de clientes activos
...
// Lista de clientes activos con tabla de datos
export function ClientsActive({ onDataLoaded }: ClientsActiveProps) {
  ...
  return (
    <View className="flex-1 px-3 pt-3">
      <Text className="text-lg font-bold text-text mb-1">Clientes activos</Text>
      <Text className="text-sm text-text/65 mb-3">
        {pagination.totalItems > 0
          ? `${pagination.totalItems} activos`
          : 'Cargando...'}
      </Text>
